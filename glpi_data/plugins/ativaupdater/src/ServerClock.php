...
/**
 * Wall-clock timestamps shared by the dashboard and the service API.
 *
 * With use_timezones enabled, GLPI switches PHP's default timezone to the
 * logged-in user's preference, while the stateless service API keeps the
 * server default. Dates written with date() on each side are then not
 * comparable (a "Verificar agora" request could look older than the last
 * acknowledgement). Plugin timestamps therefore always use the server
 * timezone configured in php.ini.
 */
