...
GLPI_URL = "https://10.117.41.6:8443/apirest.php"
...
def obter_sessao_glpi():
    ...
    res = requests.get(f"{GLPI_URL}/initSession", headers=headers, verify=False)
    if res.status_code == 200:
...
def sincronizar_usuarios(usuarios_ms, session_token):
    ...
    for ms_user in usuarios_ms:
        ...
        res = requests.post(f"{GLPI_URL}/User", headers=headers, json=payload, verify=False)
